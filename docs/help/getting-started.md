...
4. Type in a name like `Metal`.
5. Fill in a description if you'd like.
6. Tap **Save**.

For this example you'll need the following categories: `Metal`, `Materials`, `Wood`, `Gear`. 

> **Tip:** Adding multiple items with near-identical values? Copy 'em!  
> ![Copy](assets/content-copy.svg) **Copy** opens the editor pre-filled from the card you pick, with 
> '- Copy' added to the name. So, copy, change the name, save, and do it again. It's that easy!
>
> You can copy Categories, Components, or Blueprints. See [Copying a record](dataset.md#copying-a-record) for more 
> info.
...
4. **Production time per item**: how long one takes to gather or smelt. Optional. Leave it at zero and
   it reads as *Instant*. Or, just guesstimate it. This will allow you to calculate how long it would take you to 
   grind out the raw materials you need.
5. **Category**: `Metal`
6. **Save**.
...
> and 40 seconds, and that you will need 800 Wood and 320 Stone. You could create the full stack of arrows as 
...
8 Tin on top of the Wood and Leather Scraps. And it also knows that crafting your Bronze Axe is a 9-step process. 8 
steps at the forge for Bronze, and then a final step for the Axe itself.
...
the **Steps** tab for the whole tree, in order.
